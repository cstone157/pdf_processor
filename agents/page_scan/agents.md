...
1. **Chunking Constraints**: 
   ...
   - Sections that don't contain any text block, shouldn't be added.
2. **Exclusion Criteria**:
   - Strictly exclude any tables, figures, or diagrams that are explicitly labeled (e.g., "Figure 1," "Table A-1").
   - Retain lists (bulleted or numbered) and unlabeled data tables that are embedded within the text flow. Lists and unlabeled tables should be stored in their own objects.
3. **Keyword Extraction**:
   - Extract 1–10 highly relevant technical keywords or phrases per chunk for indexing.
4. **Section Mapping**:
